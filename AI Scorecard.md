...
| **Knowledge Graph** | 91/100 | Particularly good because it is operational memory, not just visualisation. Caller/callee retrieval and mappings make it useful to agents. |
...
| **Main IDE functionality** | 88/100 | Quite capable for a dependency-light Tkinter IDE. Editor, structure, console, project tree, planning chat and run tooling make it a real environment rather than a demo shell. |
| **UI/UX** | 82/100 | Much improved, but still the area where the architecture occasionally shows through. Wrapper/enhancement behaviour, window state, agent controls and some workflow discoverability could be smoother. |
| **Reliability / defensive coding** | 86/100 | Atomic writes, drift protection, staged edits, frozen files and repair gates are excellent. There are still edge cases to harden. |
...
| **Differentiation** | 95/100 | Probably Iterize's greatest strength. The persistent specification, designation mechanics, structural memory, staged agent changes and independent verification form a recognisable philosophy. |
